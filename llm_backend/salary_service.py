...
def get_salary_and_provenance(user_role, query, salaries, fuzzy_any):
    ...
    # CTO logic
    if user_role == 'Olivia Zhang (CTO)':
        # CTO can see all Technology department salaries
        tech_patterns = [
            'technology salaries', 'tech salaries', 'engineering salaries', 'list all technology salaries', 'list all tech salaries', 'list all engineering salaries',
            'show technology salaries', 'show tech salaries', 'show engineering salaries', 'technology department salaries', 'engineering department salaries'
        ]
        ...
        # CTO: all/technology salaries
        if (
            re.search(r"technology salaries", query_lc)
            or re.search(r"list all technology salaries", query_lc)
            or fuzzy_any(tech_patterns, query_lc, cutoff=0.7)
            or re.search(r"all salaries", query_lc)
            ...
            if not filtered_df.empty:
                ...
                return {"message": "No Technology salaries found.", "provenance": 'payroll_confidential.txt'}
        # CTO: self-query
        self_query_patterns = [r"show my salary", r"my salary", r"what's my salary", r"what is my salary", r"olivia zhang salary", r"show olivia zhang salary", r"show olivia zhang's salary", r"olivia zhang's salary"]
        ...
        # CTO: specific name/title queries
        if any(alias in query_lc for alias in ["cto", "chief technology officer", "olivia zhang"]):
            cto_candidates = []
            for s in salaries:
                name_lc = s[0].strip().lower()
